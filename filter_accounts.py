...
def decrypt_heimdal(secret, key):
	cmd = "/opt/mlocal/bin/decrypt %s %s" % (secret, key)
	p = Popen(shlex.split(cmd), stdout=PIPE)
	return p.stdout.read().strip()

def get_max_uid_number(ocf_ldap_url):
...
def main():
	...
	max_uid = get_max_uid_number(options.ocf_ldap_url)
	...
	for user in good_users:
		...
		entry["password"] = decrypt_heimdal(entry["heimdal_secret"], entry["heimdal_key"])
